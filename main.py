...
def recommend():
    ...
    transE_top_k = transE.query_recommendation_top_k_by_user_id(user_id=440, recommend_num=k)
    print("协同过滤推荐 TopK: ")
    print(pd.DataFrame(cf_top_k, columns=["title", "grade", "recommend_num", "genres"]).to_string(index=False))
    print("语义推荐 TopK: ")
    print(pd.DataFrame(transE_top_k, columns=["title", "grade", "recommend_num", "genres"]).to_string(index=False))
    p = 5
    q = 5
    n = math.ceil(k * (p / (p + q)))
