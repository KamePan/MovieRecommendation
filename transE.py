...
class TransE:
    ...
    '''为 entity 和 relation 生成 [id，向量]的字典，其中向量随机生成并归一化'''
    ...
    '''
    epochs 指训练迭代的次数，epochs 为 50 则代表需要迭代更新参数 50 次
    nbatches 指三元组的组数，若为 400 则代表划分为 400 组
    '''
    def train(self, epochs=50, nbatches=400):
        """"""
        epoch_loss_list = dict()
        '''这里将实际三元组数量除以组数得到每组三元组的数量，即为每组大小'''
        batch_size = len(self.triple_list) // nbatches
        print("batch size: ", batch_size)
        for epoch in range(epochs):
            start = time.time()
            self.loss = 0
            '''为什么有的组需要嵌套一层 for batch in range(nbatches) 循环呢'''
            '''这里在整个三元组集随机挑选 batch_size 个元素作为负采样集合 Sbatch'''
...
if __name__ == '__main__':
    # query_semantic_recommendation_top_k_by_user_id(440, 10, 1)
    # query_merged_recommendation_top_k_by_user_id(440, 10, 1)
    predict_list = query_merged_recommendation_top_k_by_user_id(user_id=440, recommend_num=10)
    print(pd.DataFrame(predict_list))
    # entity_set, relation_set, triple_list = database.query_entity_relation_set_and_triple_list()
    # transE = TransE(entity_set, relation_set, triple_list, embedding_dim=200, learning_rate=0.001, margin=1, L1=True)
    # transE.emb_initialize()
